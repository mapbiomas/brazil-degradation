...
// set filename
var fileName = 'degradation_nativeReference_col' + collectionId + '_v' + version;
...
years_list.forEach(function(year_j) {
  ...
  biomes_name.forEach(function(biome_k) {
    ...
    ignore_classes[biome_k].forEach(function(class_m) {
      collection_i = collection_i.updateMask(collection_i.neq(class_m));
    });
    
   
    // blend edge into recipe
    edge_degrad_year = edge_degrad_year.blend(collection_i).selfMask();
  ...
  edge_degrad_year = edge_degrad_year.set('territory', 'BRAZIL')
                                      ...
                                      .set('version', version)
                                      .set('year', year_j)
                                      .set('description', 'NativeVegetation');

    
  Map.addLayer(edge_degrad_year.randomVisualizer(),{}, String(year_j));
