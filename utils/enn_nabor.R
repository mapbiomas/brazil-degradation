...
enn_worker <- function(i, year,
  ...
  logf("Starting optimized radius-based neighbor search")
  
  dmin <- rep(NA_real_, length(pid_all))
  n_all <- length(pid_all)
  
  # choose a practical k (do NOT use n_all)
  k_use <- min(256, n_all)
  
  for (s in seq(1, n_all, by = chunk_size)) {
    
    idx <- s:min(s + chunk_size - 1, n_all)
    ...
    # Remove neighbors beyond radius
    nbr_ids[nbr_dist > searchRadius_m] <- NA
    nbr_dist[nbr_dist > searchRadius_m] <- NA
    
    # Convert to fragment IDs
    nbr_pid <- matrix(
      pid_all[nbr_ids],
      nrow = nrow(nbr_ids)
    )
    ...
    diff_frag <- nbr_pid != pid_all[idx]
    nbr_dist[!diff_frag] <- NA
    
    # Take minimum valid distance per row
    dmin[idx] <- apply(nbr_dist, 1, function(x) {
      if (all(is.na(x))) NA_real_ else min(x, na.rm = TRUE)
    })
  }
  
  logf("Radius search complete")
